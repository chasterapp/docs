...
:::info
If you intend to use the API only for yourself, you can use a [developer token](/api-developer-token) that will be linked to your account, which is easier to set up.
:::
...
- **scope=profile**: specifies [the scopes](/api-scopes) your application is requesting, separated by a space
