...
The Penalties API allows your extension to integrate with the Penalties extension, creating habits (actions) that wearers must perform while locked. This can be used to enforce regular check-ins, task completion, or any other time-based or recurring actions, adding accountability, consequences, and structured routines to your extension's features.

To learn how Penalties work from a user perspective, see the [Penalties extension documentation](/extensions/penalties).
...
#### Time-limited habits
Time-limited habits are designed for actions that need immediate attention. When triggered, the user must complete the action within a specific timeframe. For example, you might give users 2 hours to submit a verification picture after being requested, or 30 minutes to complete a task assigned through the [Tasks API](/api/extensions-api/interact-with-extensions/tasks). If the action isn't completed within the time limit, a penalty will be applied.
