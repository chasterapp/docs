...
```json
{
  "task": {
    ...
    "points": 10,
    "requireVerificationPicture": false,
    "duration": 0
  },
  "actor": "extension"
}
...
- **task** (required): The task object containing task details
  ...
  - **requireVerificationPicture** (boolean, optional): Whether the task requires photo proof. If not set, the task will not require verification pictures.
...
When `requireVerificationPicture` is `true`:
...
:::important
If you set `requireVerificationPicture` to `true`, you will not be able to complete the task using the API. Only the wearer can complete tasks that require verification pictures through the Tasks extension interface.
:::
