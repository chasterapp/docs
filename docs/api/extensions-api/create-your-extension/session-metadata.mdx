...
- `PUT /api/extensions/sessions/:sessionId/metadata`: Update metadata for an extension lock session.

To explore the documentation for the metadata endpoint and view the API specifications, visit [the API documentation](https://api.chaster.app/api#/Partner%20Extensions/PartnerActionsController_setMetadata).
