...
The timer corresponding to the remaining time will be displayed when enabled. When disabled, the timer will be hidden. If the lock has a keyholder, only he will be able to see the remaining time. The keyholder can also enable or disable the timer display if the lock's [permissions](/locks/permissions) allow it, which they do by default.
...
The time information is displayed by default. Once the lock has started, changing this setting is governed by the lock's [permissions](/locks/permissions); by default, the keyholder can show or hide time values in the history.
...
The keyholder can change the minimum lock time if the lock's [permissions](/locks/permissions) allow it, which they do by default. In any case, it cannot be set above the maximum lock time defined by the **Limit lock time** option.
