...
To enable verification for tasks, check the "Enable verification" option. You can choose who can validate your submissions:

- **Community members**: Allows other members to review and validate your task completion images through a voting process
- **Only my keyholder**: Restricts validation to your keyholder only, keeping the verification process private

You can also configure the penalty that will be applied when your verification is rejected.

In addition to a verification picture, individual tasks can require a **verification code**. When a task with this option is assigned, a unique six digit code is generated. You must include this code in your verification picture, next to the proof of task completion. You can write it on paper or display it on a device screen. This proves that the photo was taken for this specific task, after it was assigned.

When using community verification, a verification request lasts up to the configured voting duration (6 hours by default, and it can end earlier if enough votes are cast). If you complete a task with community verification enabled and task points configured, your points are granted once the community validates your submission. With keyholder verification, you'll need to wait for your keyholder to review and approve your submission. See the [Verification](/features/verification) documentation for how the voting window is configured.

For more information about how verification works, including the validation process and requirements for each mode, refer to the [Verification](/features/verification) documentation.
...
- **Verification picture required**: If verification is enabled, you can require a verification picture for this specific task. When enabled, you must upload and have your verification picture approved before the task can be marked as complete.
- **Verification code required**: If the task requires a verification picture, you can also require a verification code. A unique six digit code is generated when the task is assigned, and it must be clearly visible in your verification picture.
...
- The verification code to include in your picture (if a code is required)
...
2. If verification is required for the task, upload a verification picture demonstrating task completion. If the task requires a verification code, make sure the code is clearly visible in your picture.
3. If verification is enabled and required, wait for validation:
   - **Community verification**: Wait for the community to validate your submission. Verification lasts up to the configured voting duration, so your points will be granted once the verification is accepted.
   - **Keyholder verification**: Wait for your keyholder to review and approve your submission.
