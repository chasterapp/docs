...
You can configure the frequency of a recurring action, which can be set to a specific number of minutes (from 5 to 55 minutes, in steps of 5), hours (ranging from 1 hour to 23 hours) or days (ranging from 1 day to 30 days). By default, the frequency is set to every day, but you can customize the repeat interval to fit your needs.

Minute frequencies are useful for short locks of a few hours, where even an hourly action would be too spread out.
...
For hourly and daily actions, you can choose how the penalty periods reset:
...
Recurring actions **in minutes** always reset based on the moment the action is configured: periods run back to back from that time (for example 2:23 PM to 2:33 PM for a 10-minute frequency). A fixed reset time is not available for minute frequencies.
